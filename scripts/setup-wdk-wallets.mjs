...
const displayCli = process.env.WDK_CLI_BIN ? executable : "npm exec -- wdk";
...
console.log(`  ${displayCli} wallet unlock --name ${client} --ttl 5`);
console.log(`  ${displayCli} wallet unlock --name ${business} --ttl 5`);
console.log("\nDespués verificá direcciones y saldos en Sepolia:");
console.log(`  ${displayCli} get address --network sepolia --wallet ${client}`);
console.log(`  ${displayCli} get address --network sepolia --wallet ${business}`);
console.log(`  ${displayCli} get balance --network sepolia --token usdt --wallet ${client}`);
...
function create(name) {
  ...
  if (result.status !== 0) {
    console.error(`No se pudo crear ${name}. Si ya existe, no la borres: verificá con 'npm exec -- wdk wallet list'.`);
...
function run(args) {
  ...
  if (result.error || result.status !== 0) {
    console.error("WDK CLI local no está disponible. Ejecutá npm install antes de crear las wallets.");
    process.exit(1);
  }
}
